...
IMAGE MARKERS:
• Вмъквай <!-- IMAGE:1 --> и <!-- IMAGE:2 --> маркери след ПЪРВИТЕ ДВА <h2> заглавия
• Пример: <h2>Първи Метод</h2>\n<!-- IMAGE:1 -->\n<p>Текст...</p>
• НЕ слагай IMAGE markers след всички h2 - само след първи и втори
• Hero image се добавя автоматично, не го включвай
• IMAGE:1 = второ изображение, IMAGE:2 = трето изображение
...
- <!-- IMAGE:1 --> и <!-- IMAGE:2 --> след първи 2 <h2> заглавия
- Няма <h1> - заглавието идва отделно

НЕ ВКЛЮЧВАЙ hero image или други images URL-и - само маркери (TOC, IMAGE:1, IMAGE:2) където посочих.`;
...
1. Hook въведение → 100-150 думи (интригуваща история)
2. TL;DR → 50-80 думи (3-5 bullets)
3. Основно съдържание → ${Math.floor(targetWordCount * 0.6)} ДУМИ минимум!
   - Раздели на 4-6 <h2> секции
   - Всяка секция: ${Math.floor((targetWordCount * 0.6) / 5)}-${Math.floor((targetWordCount * 0.6) / 4)} думи
   - Детайлни обяснения, примери, анекдоти
4. "Как да използваш това" → ${Math.floor(targetWordCount * 0.15)} ДУМИ минимум!
   - Конкретни стъпки, не общи съвети
5. FAQ → 250-400 думи (3-5 Q&A, детайлни отговори)
6. Заключение → 100-150 думи (емоционален призив)

ПРИМЕР за ${targetWordCount} думи статия:
- Въведение: 120 думи
- TL;DR: 60 думи
- Основно (5 секции): ${Math.floor(targetWordCount * 0.6)} думи total = ${Math.floor((targetWordCount * 0.6) / 5)} думи/секция
- Практична: ${Math.floor(targetWordCount * 0.15)} думи
- FAQ: 300 думи
- Заключение: 120 думи

ПРЕДИ ДА ВЪРНЕШ ОТГОВОРА:
✓ Провери дали имаш ПОНЕ ${targetWordCount} думи
✓ Ако са под ${Math.floor(targetWordCount * 0.9)}, добави още съдържание
✓ Разшири секциите с повече примери, детайли, истории
`;
      break;
